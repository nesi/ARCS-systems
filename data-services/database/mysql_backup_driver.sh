...
fail_address=wil240@csiro.au
success_address=gareth.williams@arcs.org.au
...
if grep -qi error $TMPDIR/mysql_backup.log || test $STATUS -gt 0; then
 mail -s "ERROR! mysql backup failed on `/bin/hostname`" $fail_address < $TMPDIR/mysql_backup.log
else
