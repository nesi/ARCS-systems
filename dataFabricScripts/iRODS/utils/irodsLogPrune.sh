...
# also keep the 4 newest AutoReplicator and BacklogReplicator files.
( ls -1t        $1/server/log/rodsLog.[0-9][0-9][0-9][0-9].[0-9]* |sed -n '5,$p'
  ...
  ls -1t $1/server/log/AutoReplicator.log*                        |sed -n '5,$p'
  ls -1t $1/server/log/BacklogReplicator.log*                     |sed -n '5,$p'
) 2>/dev/null |
